ScriptName UILIB_GRIMY Extends Form
{SkyUILib API - Grimy Version}
...
Event OnKeyDown(Int KeyCode)
	IF listMenuOpen
		IF KeyCode == 256
			SendModEvent("UILIB_1_listMenuClose", "", UI.GetInt("CustomMenu", "_root.listDialog.menuList.listState.activeEntry.itemIndex") as Float)
			UI.CloseCustomMenu()
			UnregisterForAllKeys()
			listMenuOpen = false
		ELSEIF KeyCode <= 10
			SendModEvent("UILIB_1_listMenuClose", "", (KeyCode - 2) as Float)
			UI.CloseCustomMenu()
			UnregisterForAllKeys()
			listMenuOpen = false
		ENDIF
